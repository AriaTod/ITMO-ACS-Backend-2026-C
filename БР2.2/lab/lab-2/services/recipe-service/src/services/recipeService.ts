...
export async function deleteRecipe(id: number, currentUser: CurrentUser) {
  ...
  // Сначала просим Social Service удалить лайки/избранное/комментарии этого рецепта —
  // они в чужой базе (social_db), сами мы их удалить не можем. Если Social Service
  // недоступен — прерываем удаление целиком, чтобы не оставить "осиротевшие" данные.
  await socialClient.deleteInteractions(id);
