...
function proxyTo(target: string) {
  return createProxyMiddleware({
    target,
    changeOrigin: true,
    // Express обрезает часть пути, совпавшую с app.use(mountPath, ...), из req.url —
    // например, при app.use('/api/auth', ...) до прокси долетает уже "/register",
    // а не "/api/auth/register". req.originalUrl Express никогда не трогает —
    // пересылаем именно его, чтобы нижестоящий сервис получил тот же путь,
    // который прислал клиент.
    pathRewrite: (_path, req) => (req as Request).originalUrl,
    on: {
      error: (err, _req, res) => {
        console.error(`⚠️  Не удалось достучаться до ${target}:`, err.message);
...
// В обычной разработке (ts-node-dev) файл лежит на 3 уровня выше src/. В Docker
// структура папок другая (см. Dockerfile) — там путь передаётся явно через OPENAPI_PATH.
try {
  const openapiPath = process.env.OPENAPI_PATH || path.join(__dirname, '..', '..', '..', 'openapi.yaml');
  ...
  console.error('⚠️  Не удалось загрузить openapi.yaml для Swagger UI:', (err as Error).message);
}
